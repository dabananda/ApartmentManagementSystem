...
public class FullPayTenantBillCommandHandler(ITenantRentRepository repository)
    : IRequestHandler<FullPayTenantBillCommand, (bool success, string message, List<TenantPayment> payments, string? tenantUserId)>
{
    public async Task<(bool success, string message, List<TenantPayment> payments, string? tenantUserId)> Handle(FullPayTenantBillCommand request, CancellationToken cancellationToken = default)
    {
