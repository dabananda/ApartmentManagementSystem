...
public class PayAllTenantBillsCommandHandler(ITenantRentRepository repository)
    : IRequestHandler<PayAllTenantBillsCommand, (bool success, string message, List<TenantPayment> payments, string? tenantUserId)>
{
    public async Task<(bool success, string message, List<TenantPayment> payments, string? tenantUserId)> Handle(PayAllTenantBillsCommand request, CancellationToken cancellationToken = default)
    {
        ...
        if (created.Count == 0) return (false, "Nothing due to pay or no bills found.", [], request.TenantUserId);
