...
public class PayAllOwnerBillsCommandHandler(IOwnerBillingRepository repository)
    : IRequestHandler<PayAllOwnerBillsCommand, (bool success, string message, IEnumerable<ExpenseAllocationPayment> payments)>
{
    public async Task<(bool success, string message, IEnumerable<ExpenseAllocationPayment> payments)> Handle(PayAllOwnerBillsCommand request, CancellationToken cancellationToken = default)
    {
        var created = await repository.RecordPayAllAsync(request.OwnerId, request.RestrictToBuildingId, cancellationToken);
        if (created.Any() == false) return (false, "Nothing due to pay or no bills found.", []);
