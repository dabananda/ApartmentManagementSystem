...
public sealed class TenantDirectoryRepository(ApplicationDbContext context) : ITenantDirectoryRepository
{
    ...
    {
        var assignmentRows = await context.TenantAssignments.Include(assignment => assignment.Flat)!.ThenInclude(flat => flat.Owner).Include(assignment => assignment.TenantUser)
        ...
        var legacyRows = await context.Tenants.Include(tenant => tenant.Flat)!.ThenInclude(flat => flat.Owner)
