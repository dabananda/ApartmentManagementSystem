...
namespace ApartmentManagementSystem.Infrastructure.Repositories.Tenancy;
...
public sealed class FlatBillingProfileRepository(ApplicationDbContext context) : IFlatBillingProfileRepository
{
    ...
    {
        var flats = context.Flats.AsQueryable(); if (ownerId is not null) flats = flats.Where(flat => flat.OwnerId == ownerId);
    ...
    public Task<TenantAssignment?> GetCurrentAssignmentAsync(Guid flatId, DateTime today, CancellationToken cancellationToken = default) => context.TenantAssignments.Where(assignment => assignment.FlatId == flatId && (assignment.EndDate == null || assignment.EndDate >= today)).OrderByDescending(assignment => assignment.StartDate).FirstOrDefaultAsync(cancellationToken);
